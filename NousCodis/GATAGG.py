...
data = np.load(r'/fhome/pmarti/TFGPau/novesDades.npz', allow_pickle=True)
...
data = np.load(r'/fhome/pmarti/TFGPau/featuresNoHosp.npz', allow_pickle=True)
...
for fold, (tr, va) in enumerate(skf.split(f_no, y_no_hosp, PatID_no_hosp),1):
    print(f"\n--- Fold {fold} ---")
    model = GATWeight(768,256,2,use_edge_attr=False).to(device)
