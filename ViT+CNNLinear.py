...
data2 = np.load('/fhome/pmarti/TFGPau/DBLarge_FeatMatNew_norm_ho.npz', allow_pickle=True)
...
n_splits = 5
sgkf = StratifiedGroupKFold(n_splits=n_splits, shuffle=True, random_state=2)
...
for fold, (train_idx, val_idx) in enumerate(sgkf.split(X_no_hosp, y_no_hosp, groups=PatID_no_hosp)):
    ...
    # Entrenar y validar
    model, t_loss = train_model(train_loader, val_loader, metrics, model, epochs=7)
...
np.savez("/fhome/pmarti/TFGPau/NPZs/ViT+CNNLinear.npz", 
