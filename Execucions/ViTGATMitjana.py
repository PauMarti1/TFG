...
def attn_to_graph(attn_matrices, threshold=0.0):
    # Combina les 12 capes d'atenció fent mitjana
    adj = attn_matrices.mean(axis=0)  # shape: (196, 196)
    mask = adj > threshold
    edge_index = np.array(mask.nonzero())  # shape: (2, num_edges)
    edge_attr = adj[mask].reshape(-1, 1)  # shape: (num_edges, 1)
    return edge_index, edge_attr

threshold = 0.0  # o el que vulguis
adj_list = []
edge_attr_list = []
for i in range(len(attn_matrices)):
    ...
    edge_weight = torch.tensor(edge_weight_np, dtype=torch.float).view(-1, 1)
...
for fold, (train_idx, val_idx) in enumerate(skf.split(features_list, y_no_hosp, groups=PatID_no_hosp)):
    ...
    gat       = GAT(768, 256, 2, use_edge_attr=False).to(device)
